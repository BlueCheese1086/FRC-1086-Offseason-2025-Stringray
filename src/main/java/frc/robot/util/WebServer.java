...
public class WebServer extends NanoHTTPD {
  ...
  private volatile int lastCircleClicked = -1;
  private volatile String lastLevelSelected = null;

  public int getLastCircleClicked() {
    return lastCircleClicked;
  }

  public String getLastLevelSelected() {
    return lastLevelSelected;
  }
  ...
  public Response serve(IHTTPSession session) {
    ...
    if (uri.equals("/update")) {
      try {
        lastCircleClicked = Integer.parseInt(params.get("circle"));
        lastLevelSelected = params.get("level");
        System.out.println(
            "WEB UPDATE → Circle " + lastCircleClicked + " Level " + lastLevelSelected);
    ...
    html.append("<!DOCTYPE html>")
        ...
        .append(circleSize)
        .append(
            "px; border-radius:50%; background:#9d4bff; position:absolute; cursor:pointer; transition:0.2s; }")
        .append(".circle.selected { background:#3aff6f; }")
        .append(
            ".popup { display:none; position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); background:black; padding:20px 30px; border-radius:10px; border:2px solid white; }")
        .append(
            ".popup button { margin:6px; padding:8px 15px; font-size:16px; cursor:pointer; border-radius:6px; }")
        .append("</style>")
        .append("<script>")
        ...
        .append("    circle.classList.add('selected');")
        .append("    circle.dataset.level = level;")
        .append("  }")

        // SEND RESULT TO ROBOT
        .append("  fetch('/update?circle=' + activeCircle + '&level=' + level);")
    ...
    double rotationOffset = Math.toRadians(15) * 1;
